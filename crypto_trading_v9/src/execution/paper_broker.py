...
class PaperBroker:
    ...
    def oco_sell(self, symbol: str, qty: float, target_price: float,
        ...
        الحقيقية (orderReports). كانت النسخة السابقة تُنشئ أمراً
        واحداً بمعرّف واحد يخدم الوقف والهدف معاً — يمنع التمييز بين
        الأمرين الفرعيين تماماً كما في العميل الحقيقي، وهذا بالضبط
        الخلل الذي يمنع Reconciliation من التعرّف على كل طرف OCO على
        حدة.
        """
        ...
        stop_cid = f'{client_id}-STOP'
        target_cid = f'{client_id}-TARGET'
