...
class FakeExchange:
    ...
    def oco_sell(self, symbol, qty, target_price, stop_price, stop_limit_price, cid):
        """
        استجابة OCO واقعية — أب (orderListId) وابنان بمعرّفات مستقلة،
        مطابقة تماماً لبنية استجابة بينانس الحقيقية (orderReports).
        """
        with self._lock:
            self.calls += 1; self.send_calls += 1
            stop_cid, target_cid = f'{cid}-STOP', f'{cid}-TARGET'
