...
class LiveTrader:
    def __init__(self, envcfg, cfg: Config, max_notional: float):
        ...
        if self.env is Env.SHADOW:
            ...
            self.paper = PaperBroker(self.db, self.public, costs=costs,
    ...
    def _settle_paper_trigger(self, trig: Dict):
        ...
        row = self.db.get_intent_by_client_order_id(trig['cid'])
        if not row or not row.get('position_id'):
            return
        pos_rows = self.db.query('SELECT * FROM positions WHERE id=?',
                                 (row['position_id'],))
        if not pos_rows or pos_rows[0]['status'] != 'OPEN':
            return
        pos = pos_rows[0]
        resp = self.paper.order_by_client_id(self.symbol, trig['cid'])
        self.db.transition_order_state(trig['cid'], 'FILLED',
...
def main():
        ...
        if a.mode == 'recon':
            ...
            if c.env is Env.PAPER:
                ...
                costs = PaperCosts(
                    ...
                    latency_ms=int(os.getenv('PAPER_LATENCY_MS', '250')),
                    partial_fill_ratio=float(os.getenv('PAPER_PARTIAL_RATIO', '0')))
                cl = PaperBroker(db, public, costs=costs,
                                 starting_quote=float(os.getenv('CAPITAL', '1000')),
                                 env_tag=c.tag)
    ...
    try:
        t = LiveTrader(c, cfg, a.max_notional)
    except Exception as e:
