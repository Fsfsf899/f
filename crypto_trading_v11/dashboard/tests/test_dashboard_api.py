...
# ══ 7. مسح أفضل فرصة — الأقسام 86/92/93 ══
class Test07_OpportunityScans(Base):
    """القسم E من متطلبات V11 الأخيرة: اللوحة/API لمحرك اختيار أفضل فرصة."""
    ...
    def setUpClass(cls):
        ...
        from src.storage.database import Database as _WriteDB
        from src.selection.opportunity import RankingResult, OpportunityScore
        db = _WriteDB(cls.path)
        result = RankingResult(
            decision='BUY', selected_symbol='ETHUSDT',
            reason='BEST_ELIGIBLE_OPPORTUNITY', tie_break_applied=False,
            opportunities=[
                OpportunityScore(symbol='ETHUSDT', eligible=True, decision='BUY',
                                 score=78.5, components={'signal_quality': 0.3}),
                OpportunityScore(symbol='BTCUSDT', eligible=False, decision='WAIT',
                                 rejection_reasons=['SCORE_TOO_LOW'])])
        db.save_opportunity_scan(result, btc_context='LOW')

    def test_opportunity_scans_endpoint_returns_seeded_scan(self):
        _, b = self.get('/api/opportunity-scans')
        self.assertGreaterEqual(len(b['data']), 1)
        row = b['data'][0]
        for f in ('id', 'ts', 'scanned_symbols', 'decision', 'selected_symbol',
                  'reason', 'tie_break_applied', 'btc_context', 'opportunities'):
            self.assertIn(f, row)
        self.assertEqual(row['selected_symbol'], 'ETHUSDT')

    def test_last_opportunity_scan_endpoint(self):
        _, b = self.get('/api/opportunity-scans/last')
        self.assertEqual(b['data']['selected_symbol'], 'ETHUSDT')
        self.assertEqual(b['data']['decision'], 'BUY')
        opps = b['data']['opportunities']
        self.assertEqual(len(opps), 2)
        rejected = [o for o in opps if not o['eligible']]
        self.assertEqual(len(rejected), 1)
        self.assertIn('SCORE_TOO_LOW', rejected[0]['rejection_reasons'])

    def test_opportunity_scans_is_read_only(self):
        r = self.c.post('/api/opportunity-scans')
        self.assertEqual(r.status_code, 405)

    def test_opportunity_scans_limit_is_capped(self):
        _, b = self.get('/api/opportunity-scans?limit=999999')
        # لا يجوز أن يُعيد أكثر من MAX_LIMIT — لا نتحقق من رقم بعينه
        # هنا، فقط أن الطلب لا يفشل ويُعيد قائمة
