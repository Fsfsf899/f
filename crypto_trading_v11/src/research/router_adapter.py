...
class RouterAsSignalEngine:
    ...
    def evaluate(self, data, idx: int, *, data_quality: float = 1.0,
        ...
        context = {'prep': prep or self.prepare(data),
                  ...
                  # ⚠️ إصلاح حرِج (تدقيق V11 Final Patch، القسم 12):
                  # btc_ctx كانت تُبتلَع بصمت عبر **kw هنا — لا تصل أبداً
                  # إلى context التي يقرأها BaselineEntryModel/
                  # BreakoutEntryModel/PullbackEntryModel فعلياً
                  # (context.get('btc_ctx')). بما أن live_trader.py
                  # يستخدم RouterAsSignalEngine حصراً لا SignalEngine
                  # المباشرة، كان إصلاح فشل-الإغلاق عند غياب BTC
    ...
    **نموذجاً واحداً فقط**، بلا سقوط لـ Baseline إطلاقاً — WAIT صريح
    ...
    def evaluate(self, data, idx: int, *, data_quality: float = 1.0,
        ...
        context = {'prep': p, 'data_quality': data_quality,
                  'account_state': kw.get('account_state', {}),
                  # نفس إصلاح RouterAsSignalEngine أعلاه — btc_ctx كانت
                  # تُبتلَع بصمت عبر **kw هنا أيضاً.
                  'btc_ctx': kw.get('btc_ctx'), 'spread_bps': kw.get('spread_bps')}
