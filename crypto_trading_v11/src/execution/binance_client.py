...
class BinanceClient:
    ...
    def oco_sell(self, symbol: str, qty: float, target_price: float,
        ...
        """
        OCO عبر `POST /api/v3/orderList/oco` — الآلية الحالية المدعومة.
        `POST /api/v3/order/oco` القديمة **مهجورة رسمياً** (تحقَّقت من
        هذا فعلياً عبر توثيق بينانس الرسمي، لا افتراضاً — الإهجار بدأ
        بالتطبيق مارس 2026). العقدان مختلفان فعلياً، لا تغيير مسار فقط:

        القديم: side ثابت SELL، price/stopPrice/stopLimitPrice مباشرة.
        الجديد: `aboveType`/`belowType` صريحان — "above" هو الأمر فوق
        السعر الحالي (`LIMIT_MAKER` = هدفنا)، "below" هو الأمر تحته
        (`STOP_LOSS_LIMIT` = وقفنا)، وهذا يطابق قيد السعر الموثَّق
        رسمياً: `LIMIT_MAKER price > السعر الحالي > STOP_LOSS stopPrice`
        لجهة SELL — بالضبط حالتنا (حماية مركز Long).

        `above_client_id`/`below_client_id`: **لا نعتمد على تخمين
        بينانس أو أي لاحقة ضمنية** — نُرسل معرّفَي الأبناء صراحةً
        (`aboveClientOrderId`/`belowClientOrderId`) ونتحقق من مطابقة
        ما تُعيده المنصة لهما بالضبط في طبقة التحليل (`OCOResult`).
        """
        params = {
            'symbol': symbol, 'side': 'SELL',
            'quantity': self.round_qty(symbol, qty),
            'aboveType': 'LIMIT_MAKER',
            'abovePrice': f"{self.round_price(symbol, target_price)}",
            'belowType': 'STOP_LOSS_LIMIT',
            'belowStopPrice': f"{self.round_price(symbol, stop_price)}",
            'belowPrice': f"{self.round_price(symbol, stop_limit_price)}",
            'belowTimeInForce': 'GTC',
            'listClientOrderId': client_id,
        }
        if above_client_id:
            params['aboveClientOrderId'] = above_client_id
        if below_client_id:
            params['belowClientOrderId'] = below_client_id
        return self._req('POST', '/api/v3/orderList/oco', params, signed=True)

    def cancel_oco(self, symbol: str, *, order_list_id: Optional[str] = None,
                   list_client_order_id: Optional[str] = None) -> Dict:
        """
        إلغاء القائمة الكاملة عبر `DELETE /api/v3/orderList` — العملية
        القانونية الوحيدة لإلغاء OCO (توثيق بينانس: إلغاء طرف واحد عبر
        `/api/v3/order` يُلغي القائمة بأكملها ضمنياً، لكن الصريح أوضح
        ولا يعتمد على ذلك السلوك الجانبي).
        """
        ...
        if order_list_id:
            p['orderListId'] = order_list_id
        if list_client_order_id:
            p['listClientOrderId'] = list_client_order_id
        return self._req('DELETE', '/api/v3/orderList', p, signed=True)

    def query_oco(self, *, order_list_id: Optional[str] = None,
                  list_client_order_id: Optional[str] = None) -> Dict:
        """استعلام القائمة — `GET /api/v3/orderList`."""
        if not order_list_id and not list_client_order_id:
            raise ValueError('يلزم order_list_id أو list_client_order_id')
        p: Dict[str, Any] = {}
        if order_list_id:
            p['orderListId'] = order_list_id
        if list_client_order_id:
            p['origClientOrderId'] = list_client_order_id
        return self._req('GET', '/api/v3/orderList', p, signed=True)
