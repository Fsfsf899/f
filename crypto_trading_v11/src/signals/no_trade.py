...
R = {
    ...
    'BTC_UNAVAILABLE':     'BTC_DATA_UNAVAILABLE',
...
class NoTradeEngine:
    ...
    def check(self, *, data_quality: float, bars_available: int, warmup: int,
        ...
        if c.require_btc_ok:
            # القسم 12 من متطلبات V11 Final Patch: "missing BTC data
            # -> NO TRADE... Do not invent BTC values." — الفحص القديم
            # `btc_available and btc_ok is False` كان يتجاهل الحالة
            # التي تهمّه أكثر: BTC **غائب تماماً** لا يُمنَع التداول
            # إطلاقاً (الشرط لا يُقيَّم أصلاً بلا btc_available=True).
            # BTCContext.evaluate() نفسها تُعيد trend_ok=True كقيمة
            # افتراضية للحقل عند غياب البيانات (إيجابي صامت) — فحص
            # هنا على مستوى البوابة يمنع الاعتماد على تلك القيمة
            # الافتراضية أصلاً بدل إصلاحها في مصدرين مختلفين.
            if not btc_available:
                v.block(R['BTC_UNAVAILABLE'])
        ...
        # القسم 6 من متطلبات V11 Final: "min_confidence وmin_probability
        # يجب أن تتدفق فعلياً إلى محرك القرار — لا إعدادات ميتة". كانتا
        # معرَّفتين في Config بلا أي استخدام إطلاقاً — الفحص الفعلي كان
        # يقارن confidence بقيمة مُشتقة عشوائياً (min_data_quality*0.6)،
        # وprobability برقم مكتوب حرفياً (0.40) يطابق القيمة الافتراضية
        # لـ min_probability بالصدفة فقط دون ارتباط حقيقي — أي تغيير
        # مستقبلي في min_probability كان سيُهمَل تماماً. الافتراضي None
        # يحافظ على توافق خلفي كامل لأي استدعاء قديم لا يُمرِّر الحدّين.
        eff_min_conf = min_confidence if min_confidence is not None else c.min_data_quality * 0.6
        if confidence is not None and confidence < eff_min_conf:
            v.block(R['CONFIDENCE'], f"{confidence:.2f} < {eff_min_conf:.2f}")
        eff_min_prob = min_probability if min_probability is not None else 0.40
        if probability is not None and probability < eff_min_prob:
            v.block(R['PROBABILITY'], f"{probability:.2f} < {eff_min_prob:.2f}")
