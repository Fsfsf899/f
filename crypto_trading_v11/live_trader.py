...
def main():
    ...
    if a.mode == 'live':
        ...
        # ⚠️ حاجز صريح (المراجعة النهائية): `LiveOrderManager` وحدوده
        # الصارمة (`LiveLimits`) مبنيّة ومُختبَرة بالكامل، لكنها **غير
        # موصولة بأي مسار إنتاج** — `LiveTrader` يبني `OrderManager`
        # العادي في كل الأوضاع بما فيها `live`. الفارق ليس تجميلياً:
        #
        #     مخاطرة لكل صفقة   1.00%  بدل  0.25%
        #     سقف قيمة المركز   25% من الحقوق  بدل  $25 مطلقاً
        #     التعرّض الكلي     60%  بدل  10%
        #
        # أي أن طبقة الأمان المكتوبة **خصّيصاً للمال الحقيقي** لا تعمل
        # في مسار المال الحقيقي. بوابة الجاهزية تمنع الوصول إلى هنا
        # اليوم، لكن الاعتماد على قفل واحد لعيب بهذا الأثر لا يكفي —
        # فالحاجز هنا صريح ومستقل.
        if os.getenv('I_ACCEPT_LIVE_LIMITS_NOT_WIRED') != 'yes':
            print('\n⛔ LIVE_LIMITS_NOT_WIRED')
            print('   LiveOrderManager غير موصول بمسار التنفيذ — حدود')
            print('   LiveLimits (0.25% مخاطرة، سقف $25، تعرّض 10%) لن')
            print('   تُطبَّق. سيعمل النظام بحدود Config العامة الأوسع.')
            print('   صِل LiveOrderManager أولاً، أو أقرّ بالتجاوز صراحةً.')
            return 1
